...
class ManageFriendRequestsBloc
    extends Bloc<ManageFriendRequestsEvent, ManageFriendRequestsState> {
  ManageFriendRequestsBloc(this._loggedUserId)
      : super(const ManageFriendRequestsState()) {
  ...
  Future<FutureOr<void>> _denyPressed(
    DenyPressed event,
    Emitter<ManageFriendRequestsState> emit,
  ) async {
    try {
      await _userDataService.denyFriendRequest(event.uid, _loggedUserId);
      final newRequests =
          List<FirestoreUserPublicData>.from(state.friendRequests)
            ..removeWhere((e) => e.id == event.uid);
      emit(
        state.copyWith(
          friendRequests: newRequests,
          pageStateEnum: PageStateEnum.loaded,
        ),
      );
    } catch (e) {
  ...
  Future<FutureOr<void>> _acceptPressed(
    AcceptPressed event,
    Emitter<ManageFriendRequestsState> emit,
  ) async {
    try {
  ...
  Future<FutureOr<void>> _getInitialRequests(GetInitialFriendRequests event,
      Emitter<ManageFriendRequestsState> emit) async {
    try {
