...
class FriendRequestsListWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => ManageFriendRequestsBloc(
        context.read<AuthBloc>().state.authUserData.id,
      ),
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            return FriendRequestItemWidget(
                              onTapAccept: () =>
                                  context.read<ManageFriendRequestsBloc>().add(
                                        AcceptPressed(
                                          state.friendRequests[index].id,
                                        ),
                                      ),
                              displayName:
                                  state.friendRequests[index].displayName ?? '',
